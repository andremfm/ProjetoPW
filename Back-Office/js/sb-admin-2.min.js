...
console.log(data);
  for(let i = 0;i<data.length;i++){
    if(data[i].Id == index){
       data[i].Nome = document.getElementById("FirstName").value;
       data[i].Apelido = document.getElementById("LastName").value;
       data[i].DataNasc = document.getElementById("DataNasc").value;
       data[i].Email = document.getElementById("Email").value;
       data[i].Telefone = document.getElementById("Telefone").value;
       data[i].Funcao = document.getElementById("Funcao").value;
       data[i].Salario = document.getElementbyId("Salario").value;
       data[i].Valor = document.getElementById("Valor").value;
    }
  }
...
function guardarMat() {
  ...
  if(document.getElementById("Tipo").value === ""){
    ...
    material.Preco = document.getElementById("Preco").value;
...
function loadDataMat() {
  ...
  let tableData = data.map(material => (
    ...
    <tr>
      ...
      <td class="btn btn-danger btn-sm delete" onclick="delMat(${i++});window.location.reload()">Apagar</td>
...
function loadRiscos() {
  ...
  let tableData = data.map(risco => (
    ...
    <tr>
      ...
      <td>${risco.Taxa}€</td>
      <td class="btn btn-info btn-sm">Alterar  </td>
      <td class="btn btn-danger btn-sm delete" onclick="delRiscos(${i++})">Apagar</td>
...
function delRiscos(index) {
  let data = JSON.parse(localStorage.getItem("riscos"))
  data.splice(index, 1);
  if (localStorage.getItem("riscos") == null) {
    localStorage.setItem("riscos", JSON.stringify(data));
  } else {
    localStorage.setItem("riscos", JSON.stringify(data));
  }
  loadRiscos();
...
function loadservicos() {
  ...
  let tableData = data.map(servico => (
    ...
    <tr>
      ...
      <td>${servico.Taxa}€</td>
      <td class="btn btn-info btn-sm">Alterar  </td>
      <td class="btn btn-danger btn-sm delete" onclick="delServicos(${i++})">Apagar</td>
  ...
  let tableData = data.map(operacao => (
    ...
    <tr>
      ...
      <td class="btn btn-info btn-sm" data-toggle="modal" data-target="#func-modal${i}">Detalhes</td>
        <td class="btn btn-danger btn-sm delete" onclick="delListOp(${i})">Apagar</td>
    </tr>
    <div class="modal fade" id="func-modal${i++}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
              ...
              <div class="modal-body">
                ...
                <p>Funcionarios: ${operacao.Funcionarios} (${operacao.taxaFunc}€)</p>
...
function delListOp(index) {
  let data = JSON.parse(localStorage.getItem("operacoes"))
  data.splice(index, 1);
  if (localStorage.getItem("operacoes") == null) {
    localStorage.setItem("operacoes", JSON.stringify(data));
  } else {
    localStorage.setItem("operacoes", JSON.stringify(data));
  }
  loadListaOperacoes();
...
function loadDataPedidos() {
  ...
  let tableData = data.map(pedido => (
    ...
    <tr>
      ...
      <td class="btn btn-danger btn-sm delete" onclick="delPedidos(${i++})">Apagar</td>
    </tr>
  `
  )).join('');
  console.log(tableData);
...
function totalOperacoes(){
  ...
  let vdesp = document.querySelector('#bannerOperacoes');
...
function valorOperacoes(){
  ...
  for(var i = 0; i < data.length; i++){
    let r = parseFloat(data[i].TaxaRisco);
    let s = parseFloat(data[i].TaxaServico);
    let f = parseFloat(data[i].taxaFunc);
    let m = parseFloat(data[i].taxaMat);
    
    subtotal = subtotal + r + s + f + m;
  }

  total = subtotal * 1.23;

  let valorop = document.querySelector('#valorOp');
  valorop.innerHTML = total + "€";
}

//Despesas material
function despesasMatFunc(){
  let data1 = JSON.parse(localStorage.getItem("materiais"))
  let data2 = JSON.parse(localStorage.getItem("funcionarios"))

  console.log(data1)
  console.log(data2)
  let subtotal1 = 0
  for (let i=0; i<data1.length; i++) {
    subtotal1 += parseInt(data1[i].Preco);
}
let subtotal2 = 0
  for (let i=0;i<data2.length; i++){
    subtotal2 += parseInt(data2[i].Salario);
  }
  if(subtotal1==null){
    subtotal1=0;
  }else if(subtotal2 == null){
    subtotal2=0;
  }
  let total = 0;
...
function totalFuncionarios(){
  ...
  for(var i = 0; i<data.length; i++){
    if(data[i].Funcao == "Reporter"){
      r = r + 1;
    }else if(data[i].Funcao == "Cameraman"){
      c = c + 1;
    } else {
      f = f + 1;  
    }
  ...
  let card = (
    ...
    <p>Cameraman's: ${c}</p>
    <p>Reporters: ${r}</p>
    <p class="font-weight-bold">Total: ${f + c + r}</p>
    `
  );

  let dados = document.querySelector('#cardindex');
  dados.innerHTML = card;
}
...
function locaisOp(){
  ...
  let Data = data.map(operacao => (
      `  
      <p>${operacao.Local}</p>
  `
  )).join('');
  console.log(Data);
  let tbody = document.querySelector('#locaisOp');
