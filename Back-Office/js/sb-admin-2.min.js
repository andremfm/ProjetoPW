...
function loadDataOperacoes() {
  ...
  let selectDataSer = dataSer.map(servico => (
    `
    <option value="${servico.Taxa}">${servico.Tipo} (${servico.Taxa}€)</option>
`
)).join('');
...
let dataRisco = JSON.parse(localStorage.getItem("riscos"))
  ...
  let selectDataRisco = dataRisco.map(risco => (
    `
    <option value="${risco.Taxa}">${risco.Tipo} (${risco.Taxa}€)</option>
`
)).join('');
...
function registarOp(){
  ...
  let s = document.getElementById("tipoServico").value;
  let r = document.getElementById("nivelRisco").value;

  sessionStorage.setItem("Evento", JSON.stringify(e));
  sessionStorage.setItem("Pais", JSON.stringify(p));
  sessionStorage.setItem("Servico", JSON.stringify(s));
  sessionStorage.setItem("Risco", JSON.stringify(r));   

}

function registarOp2(reg){
